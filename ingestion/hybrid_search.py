...
# ════════════════════════════════════════════════════════════════════════════
# 5. Pretty-print kết quả để review
# ════════════════════════════════════════════════════════════════════════════
def print_results(query: str, results: list[dict]) -> None:
    """In kết quả hybrid search ra console."""
    print("\n" + "═" * 78)
    print(f"  🔍 QUERY: {query}")
    print("═" * 78)
    ...
    for r in results:
        ...
        if r["bm25_rank"] is not None:
            src_flags.append(f"BM25 rank={r['bm25_rank']} score={r['bm25_score']}")
        if r["vector_rank"] is not None:
            src_flags.append(f"Vector rank={r['vector_rank']} score={r['vector_score']}")
        sources = "  |  ".join(src_flags)

        print(f"\n  [{r['final_rank']}]  RRF={r['rrf_score']}   ({sources})")
        print(f"       Văn bản : {r['loai_van_ban']} {r['so_hieu']}")
        print(f"       Điều {r['so_dieu']}. {r['ten_dieu'] or ''}")
        preview = (r["noi_dung"] or "")[:250].replace("\n", " ")
...
# ════════════════════════════════════════════════════════════════════════════
def main():
    # Pre-load tất cả resources trước khi search
    print("\n🚀 Hybrid Search — khởi động\n")
    _get_bm25()
    _get_model()
    _get_qdrant()

    demo_queries = [
        "Cấp độ 1 trong Điều 8. Phân loại cấp độ hệ thống thông tin sẽ là gì ?",
        "Hãy trình bày nội dung cơ bản của Điều 6. Yêu cầu về an toàn hệ thống thông tin?",
        "Chính sách của Nhà nước đối với hoạt động trí tuệ nhân tạo",
    ]

    for q in demo_queries:
        results = hybrid_search(q, final_top_k=5)
        print_results(q, results)

    print("\n🎉 Hoàn thành hybrid search!")
